...
export function useCountdown(
  ...
  const initialValues = useMemo(() => {
    performanceMonitor.start('countdown-initial-calc');

    const daysLeft = dueDate ? getDaysRemaining(dueDate) : 0;
    const timeDisplay = dueDate
      ? formatTimeDisplay(daysLeft, dueDate, status)
      : 'No due date';

    performanceMonitor.end('countdown-initial-calc');
  ...
  const dashOffset = useMemo(() => {
    performanceMonitor.start('countdown-offset-calc');
    const result = calculateTimerOffset(
      circumference,
      daysLeft,
      status,
      dueDate
    );
    performanceMonitor.end('countdown-offset-calc');
    return result;
  ...
  const updateTimeLeft = useCallback(() => {
    ...
    performanceMonitor.start('countdown-update');

    const days = getDaysRemaining(dueDate);
    setDaysLeft(days);
    setTimeDisplay(formatTimeDisplay(days, dueDate, status));

    performanceMonitor.end('countdown-update');
  }, [dueDate, status]);
