import { renderHook, describe, it, expect } from '@/test';
...
describe('useProfileValidation', () => {
  ...
  it('validates individual profile fields', () => {
    ...
    // Test valid email
    const emailValidation = result.current.validateProfileField(
      'email',
      'test@example.com'
    );
    ...
    // Test invalid email
    const invalidEmailValidation = result.current.validateProfileField(
      'email',
      'invalid'
    );
    ...
    // Test valid name
    const nameValidation = result.current.validateProfileField(
      'name',
      'Test User'
    );
    ...
    const invalidNameValidation = result.current.validateProfileField(
      'name',
      'T'
    );
  ...
  it('validates avatar URL field specifically', () => {
    ...
    expect(
      result.current.validateAvatarUrl('https://example.com/avatar.jpg')
    ).toBe(true);
