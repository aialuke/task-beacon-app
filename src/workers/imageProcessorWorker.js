...
self.onmessage = async (event) => {
  ...
  try {
    const result = await compressAndResizePhoto(file, maxWidth, maxHeight, quality);
...
async function compressAndResizePhoto(fileData, maxWidth = 1200, maxHeight = 1200, quality = 0.85) {
  // Create an image bitmap from the file data
  const arrayBuffer = await fileData.arrayBuffer();
  const bitmap = await createImageBitmap(new Blob([arrayBuffer], { type: fileData.type }));
