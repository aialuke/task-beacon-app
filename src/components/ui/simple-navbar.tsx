...
export function SimpleNavbar({ items, activeItem, onItemChange, className }: SimpleNavbarProps) {
  ...
  useEffect(() => {
    const getComputedColors = () => {
      ...
      console.log('🎨 Raw primary HSL value:', computedStyle.getPropertyValue('--primary').trim());
  ...
  const updateActiveButtonBounds = React.useCallback(() => {
  ...
  }, [items, activeItem, computedColors, isInitialized]);
  ...
  }, [activeItem, items, computedColors, isInitialized, updateActiveButtonBounds]);
  ...
  }, [activeItem, updateActiveButtonBounds]);
