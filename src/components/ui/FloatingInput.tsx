...
function isFormVariant(
  props: AuthFloatingInputProps | FormFloatingInputProps,
...
const FloatingInput = forwardRef<
  HTMLInputElement,
  AuthFloatingInputProps | FormFloatingInputProps
>((props, ref) => {
  const {
    id,
    label,
  ...
  return (
    <div className={cn('group relative', className)}>
      <div className="relative">
        ...
        {isForm ? (
          <Input
            ...
            className={cn(
              'peer h-14 bg-background/60 pb-2 pt-6 backdrop-blur-sm transition-all duration-300 hover:bg-background/70 focus:bg-background/80 focus:shadow-lg focus:shadow-primary/10 active:bg-background/75',
              icon ? 'pl-11' : 'pl-4',
          ...
          <input
            ...
            className={cn(
              ...
              error
                ...
                : 'hover:border-primary/50 focus-visible:border-primary/60 active:border-primary/60',
              disabled && 'cursor-not-allowed opacity-50',
            )}
          />
        )}
      ...
      {error && !isForm && (
        <p className="mt-1 animate-fade-in text-xs text-destructive">{error}</p>
      )}
    </div>
  );
});
