...
describe('TimerRing', () => {
  ...
  it('applies correct gradient for pending status', () => {
    const { container } = render(<TimerRing {...defaultProps} status="pending" />);
    
    const progressCircle = container.querySelector('circle[stroke="url(#gradientPending)"]');
    expect(progressCircle).toBeInTheDocument();
  });

  it('applies correct gradient for overdue status', () => {
    const { container } = render(<TimerRing {...defaultProps} status="overdue" />);
    
    const progressCircle = container.querySelector('circle[stroke="url(#gradientOverdue)"]');
    expect(progressCircle).toBeInTheDocument();
  });

  it('applies correct gradient for complete status', () => {
    const { container } = render(<TimerRing {...defaultProps} status="complete" />);
    
    const progressCircle = container.querySelector('circle[stroke="url(#gradientComplete)"]');
    expect(progressCircle).toBeInTheDocument();
  });
